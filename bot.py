...
if not BOT_TOKEN:
    raise RuntimeError("❌ Переменная BOT_TOKEN не задана! Добавь её в Railway → Variables")
ADMIN_IDS        = [5839642306]
DATA_FILE        = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "faceit_db.json")
STORAGE_CHAT_ID  = int(_os.environ.get("STORAGE_CHAT_ID", "0"))  # ID канала для хранения БД

MAPS_LIST      = ["Palace", "Miramash", "Mirage", "Almero"]
LOBBY_5V5_SIZE = 10
LOBBY_2V2_SIZE = 4
PICK_TIMEOUT   = 60
...
class Player:
    ...
    def lvl_icon(self) -> str:
        if self.elo >= 2001: return "🏆 LVL 10"
        if self.elo >= 1751: return "🔴 LVL 9"
        if self.elo >= 1531: return "🔴 LVL 8"
        if self.elo >= 1351: return "🟠 LVL 7"
        if self.elo >= 1201: return "🟠 LVL 6"
        if self.elo >= 1051: return "🟡 LVL 5"
        if self.elo >= 901:  return "🟡 LVL 4"
        if self.elo >= 751:  return "🟢 LVL 3"
        if self.elo >= 501:  return "🟢 LVL 2"
        return "⚪ LVL 1"
...
async def _bot_auto_pick(m_id: str, context: ContextTypes.DEFAULT_TYPE, chat_id: int, thread_id: Optional[int] = None):
    ...
    if phase == "pick" and m["pool"]:
        ...
        if m["pool"]:
            ...
            m["phase"] = "ban"
            m["turn"]  = t_cap   # ← Т банит первым
            # Отменяем таймер пика
...
async def _pick_timer(m_id: str, context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    """Фоновая задача: каждые 5 секунд обновляет таймер в сообщении пика."""
    while True:
...
async def start_match(players: List[int], mode: str, db: Dict,
    ...
    save_db(db)
    # Запускаем фоновый таймер (обновление каждые 5 сек)
...
async def reg_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если команда написана в группе — отправить в личку
    if update.message.chat.type in ("group", "supergroup"):
        ...
        await update.message.reply_text(
            ...
            f"<code>/reg FACEIT_ID Никнейм</code>",
            parse_mode=ParseMode.HTML
        )
        return
...
async def stats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        d = db["players"][s]
        for field, val in [("wins",0),("losses",0),("avg",0.0),("elo",1000),("external_id",""),("is_bot",False),("nickname","?"),("user_id",target)]:
            d.setdefault(field, val)
        ...
        await update.message.reply_text("❌ Вы не зарегистрированы! Используй /reg FACEIT_ID Никнейм")
...
async def listdb_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid_str, d in db["players"].items():
        ...
        lines.append(
            f"ID: <code>{uid_str}</code> | <b>{d.get('nickname','?')}</b> | FACEIT: <code>{d.get('external_id','нет')}</code>"
        )
    if len(lines) == 1:
...
def main():
    ...
    print("🤖 Бот запускается... [v7 - TOPIC FIX]")
    app.run_polling(
